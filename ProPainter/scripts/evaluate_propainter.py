...
def main_worker(args):
    ...
    if args.task == 'video_completion':
        ...
        i3d_model = init_i3d_model(
            args.i3d_model_path, device=device)
        lpips_model = None
        if not args.no_lpips:
            try:
                lpips_model = init_lpips_model(
                    net=args.lpips_net, device=device)
            except ImportError as e:
                print(f'Warning: LPIPS disabled ({e}). Install: pip install lpips')
    else:
        result_path = os.path.join(f'results_eval', 
    ...
    for index, items in enumerate(test_loader):
        ...
        if args.task == 'video_completion':
            ...
            for ori, comp in zip(ori_frames, comp_frames):
                m = compute_all_frame_metrics(
                    ori, comp,
                    lpips_model=lpips_model,
                    lpips_device=device if lpips_model is not None else None,
                )
                psnr, ssim = m['psnr'], m['ssim']

                cur_video_psnr.append(psnr)
                cur_video_ssim.append(ssim)
                cur_video_mse.append(m['mse'])
                cur_video_mae.append(m['mae'])
                if m['lpips'] is not None:
                    cur_video_lpips.append(m['lpips'])

                total_frame_psnr.append(psnr)
                total_frame_ssim.append(ssim)
                total_frame_mse.append(m['mse'])
                total_frame_mae.append(m['mae'])
                if m['lpips'] is not None:
                    total_frame_lpips.append(m['lpips'])
            ...
            cur_mse = sum(cur_video_mse) / len(cur_video_mse)
            cur_mae = sum(cur_video_mae) / len(cur_video_mae)
            cur_lpips_str = 'n/a'
            if cur_video_lpips:
                cur_lp = sum(cur_video_lpips) / len(cur_video_lpips)
                cur_lpips_str = f'{cur_lp:.4f}'

            avg_psnr = sum(total_frame_psnr) / len(total_frame_psnr)
            avg_ssim = sum(total_frame_ssim) / len(total_frame_ssim)
            avg_mse = sum(total_frame_mse) / len(total_frame_mse)
            avg_mae = sum(total_frame_mae) / len(total_frame_mae)
            avg_lpips_str = 'n/a'
            if total_frame_lpips:
                avg_lp = sum(total_frame_lpips) / len(total_frame_lpips)
                avg_lpips_str = f'{avg_lp:.4f}'

            avg_time = sum(time_all) / len(time_all)
            line = (
                f'[{index+1:3}/{len(test_loader)}] Name: {str(video_name):25} | '
                f'PSNR: {cur_psnr:.4f} SSIM: {cur_ssim:.4f} MSE: {cur_mse:.2f} MAE: {cur_mae:.2f} '
                f'LPIPS: {cur_lpips_str} | '
                f'Avg PSNR/SSIM/MSE/MAE/LPIPS: {avg_psnr:.4f}/{avg_ssim:.4f}/{avg_mse:.2f}/{avg_mae:.2f}/{avg_lpips_str} | '
                f'Time: {avg_time:.4f}'
            )
            print(line)
            eval_summary.write(line + '\n')
        else:
            avg_time = sum(time_all) / len(time_all)
            print(
    ...
    if args.task == 'video_completion':
        ...
        avg_frame_mse = sum(total_frame_mse) / len(total_frame_mse)
        avg_frame_mae = sum(total_frame_mae) / len(total_frame_mae)
        vfid_score = calculate_vfid(
            real_i3d_activations, output_i3d_activations)
        final_lpips_str = 'n/a'
        if total_frame_lpips:
            avg_lpips_final = sum(total_frame_lpips) / len(total_frame_lpips)
            final_lpips_str = f'{avg_lpips_final:.4f}'
        summary = (
            'Finish evaluation... '
            f'Avg Frame PSNR: {avg_frame_psnr:.4f} | SSIM: {avg_frame_ssim:.4f} | '
            f'MSE: {avg_frame_mse:.2f} | MAE: {avg_frame_mae:.2f} | '
            f'LPIPS: {final_lpips_str} | VFID: {vfid_score:.4f} | '
            f'Time: {avg_time:.4f}'
        )
        print(summary)
        eval_summary.write(summary)
        eval_summary.close()
    else:
        print(f'Finish evaluation... Time: {avg_time:.4f}')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--i3d_model_path', default='weights/i3d_rgb_imagenet.pt', type=str,
        help='I3D weights for VFID')
    parser.add_argument(
        '--lpips_net', default='vgg', type=str,
        choices=['alex', 'vgg', 'squeeze'],
        help='Backbone for LPIPS')
    parser.add_argument(
        '--no_lpips', action='store_true',
        help='Skip LPIPS (faster eval, no lpips package needed)')
