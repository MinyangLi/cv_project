...
def read_mask(mpath, length, size, flow_mask_dilates=8, mask_dilates=5):
    ...
    if mpath.endswith(('mp4', 'mov', 'avi', 'MP4', 'MOV', 'AVI')):
        vframes, _, _ = torchvision.io.read_video(filename=mpath, pts_unit='sec')
        masks_img = [Image.fromarray(f.numpy()) for f in vframes]
        if len(masks_img) == 0:
            raise ValueError(f'No frames in mask video: {mpath}')
        if len(masks_img) < length:
            masks_img = masks_img + [masks_img[-1]] * (length - len(masks_img))
        else:
            masks_img = masks_img[:length]
    elif mpath.endswith(('jpg', 'jpeg', 'png', 'JPG', 'JPEG', 'PNG')):  # single image
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help='Mask: single image, folder of images, or video (.mp4/.mov/.avi) with one mask frame per video frame.')
    parser.add_argument(
        '-o', '--output', type=str, default='results',
        help='Output directory. Writes <video_name>.mp4, <video_name>_masked_in.mp4 here (flat).')
    parser.add_argument(
        "--resize_ratio", type=float, default=1.0, help='Resize scale for processing video.')
    parser.add_argument(
        '--height', type=int, default=480, help='Height of the processing video.')
    parser.add_argument(
        '--width', type=int, default=864, help='Width of the processing video.')
    parser.add_argument(
    ...
    # save each frame
    if args.save_frames:
        frames_dir = os.path.join(args.output, f"{video_name}_frames")
        for idx in range(video_length):
            f = comp_frames[idx]
            f = cv2.resize(f, out_size, interpolation = cv2.INTER_CUBIC)
            f = cv2.cvtColor(f, cv2.COLOR_BGR2RGB)
            img_save_root = os.path.join(frames_dir, str(idx).zfill(4) + ".png")
    ...
    path_masked = os.path.join(args.output, f"{video_name}_masked_in.mp4")
    path_inpaint = os.path.join(args.output, f"{video_name}.mp4")
    imageio.mimwrite(path_masked, masked_frame_for_save, fps=fps, quality=7)
    imageio.mimwrite(path_inpaint, comp_frames, fps=fps, quality=7)

    print(f"\nSaved mask preview: {path_masked}")
    print(f"Saved inpaint result: {path_inpaint}")
